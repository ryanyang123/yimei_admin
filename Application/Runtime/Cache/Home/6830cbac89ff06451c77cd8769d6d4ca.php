                                    ...
                                    <table class="table table-striped table-hover" style="min-width: 860px;">
                                        ...
                                        <?php if($list): if(is_array($list)): foreach($list as $key=>$v): ?><tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <?php if($v["username"] == null): ?><span style="color: red;">未创建</span>
                                                            <?php else: ?>
                                                            <span style="color: #008000;">已创建</span><?php endif; ?>
                                                    ...
                                                    <td>
                                                        <?php if($v["user_type"] == 1): ?><span class="label label-info">普通客户</span><?php endif; ?>
                                                        <?php if($v["user_type"] == 2): ?><span class="label label-info">大客户</span><?php endif; ?>
                                                    </td>

                                                    <td>

                                                        <?php if($v["wx_name"] == null): ?><span style="color: red;">未归档</span>
                                                            <?php else: ?>
                                                            <span style="color: #008000;">已归档</span><?php endif; ?>
                                                    ...
                                                    <td>
                                                        <?php if($v["is_freeze"] == 1): ?><a href="JavaScript:EditFreeze('<?php echo ($v["id"]); ?>','0');"><span class="label label-success">正常登陆</span></a><?php endif; ?>

                                                        <?php if($v["is_freeze"] == 0): ?><a href="JavaScript:EditFreeze('<?php echo ($v["id"]); ?>','1');"><span class="label label-important">禁止登陆</span></a><?php endif; ?>

                                                    </td>
                                                    <td>

                                                        <?php if($v["add_time"] != 0): ?><a href="javascript:updatepass('<?php echo ($v["id"]); ?>','<?php echo ($v["wx_id"]); ?>')" class="btn blue"><i class="icon-key"></i> 重置密码</a><?php endif; ?>

                                                        <a href="<?php echo U('home/'.$at.'/check/edit/'.$v['id']);?>" class="btn green"><i class="icon-eye-open"></i> 查看详细</a>

<!--
                                                        <a href="JavaScript:_deleteUser('<?php echo ($v["id"]); ?>','用户列表','shop_classify');" class="btn red"><i class="icon-trash"></i> 删除</a>
-->
                                                    </td>
...
<script>
    ...
    function updatepass(id,name){
        showConfirm('<i class="icon-exclamation-sign"></i> 重置密码','确定要重置用户： '+name+' 的登陆密码吗? 重置后密码为：123456',function(){
            $.ajax({
                type: 'POST',
                url: "<?php echo U('home/'.$at.'/editpass'); ?>",
                data:{id:id},
                success: function (result) {
                    if(result.code == 1){
    ...
    function EditFreeze(id,str){
        ...
        showConfirm('<i class="icon-exclamation-sign"></i> '+name+'用户','确定要 '+name+' 该用户登陆吗?',function(){
            $.ajax({
                type: 'POST',
