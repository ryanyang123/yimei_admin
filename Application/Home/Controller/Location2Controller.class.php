...
class Location2Controller extends CommonController {
    ...
    function plus(){
        ...
        $type_list = $location_type->where(array('type'=>'2'))->select();
    ...
    public function add($name,$price,$type_id,$is_edit){
        $location = M('location');

        if(!$name){$arr = array('code'=> 0, 'res'=>'请输入下车分类名称');$this->ajaxReturn($arr,"JSON");}
        if(!$price){$arr = array('code'=> 0, 'res'=>'请输入价格');$this->ajaxReturn($arr,"JSON");}
        ...
        if($is_have){
            $arr = array('code'=> 0, 'res'=>'下车地点已存在');$this->ajaxReturn($arr,"JSON");
        }
        if($is_edit){
