...
class WebbannerController extends CommonController {
    public function _initialize(){
        if(session(XUANDB.'user_type')!='1'){
            $is_tiao = M('limits_log')->where(array('admin_id'=>session(XUANDB.'id'),'lid'=>'webbanner'))->find();
            if(!$is_tiao){
    ...
    public function add($img,$title,$info,$content,$is_edit){
        $banner = M('web_banner');
