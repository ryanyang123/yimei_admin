...
class DixiaoController extends CommonController {
    public function _initialize(){
        if(session(XUANDB.'user_type')!='1'){
            $is_tiao = M('limits_log')->where(array('admin_id'=>session(XUANDB.'id'),'lid'=>'dixiao'))->find();
            if(!$is_tiao){
    ...
    public function index(){
        ...
        $list = $tui->where($where)->order('create_time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        foreach($list as $key=>$item){

            $type_info = $hospital_order->where(array('id'=>$item['type_id']))->field('parent,order_number,id')->find();


            $list[$key]['type_info'] = $type_info;

            $user_name = $hospital->where(array('id'=>$type_info['parent']))->field('name,ali_number,ali_name')->find();
