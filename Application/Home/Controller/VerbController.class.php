...
class VerbController extends CommonController {
    public function _initialize(){
        if(session(XUANDB.'user_type')!='1'){
            $is_tiao = M('limits_log')->where(array('admin_id'=>session(XUANDB.'id'),'lid'=>'verb'))->find();
            if(!$is_tiao){
    ...
    public function index(){
        ...
        //$where['is_show'] = 1;
        $count = $home_verb->where($where)->count();
        $Page       = new \Think\Page($count,10);
        $Page->rollPage = 5;
        $Page->setConfig('theme','%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% %HEADER% ');
        $Page->setConfig('header','<p class="rows">共 %TOTAL_ROW%  个活动</p>');
        $show       = $Page->show();// 分页显示输出
        $list = $home_verb->where($where)->order('sort,create_time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function add($img,$banner_type,$url_link,$game_id,$sort,$is_edit){
        $banner = M('home_verb');
